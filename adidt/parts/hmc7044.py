...
class hmc7044_dt_layout(layout):

    template_filename = "hmc7044.tmpl"
    output_filename = "hmc7044.dtsi"
    ...
    def map_config_to_fragment(self, cfg):
        # cfg = self.make_ints(cfg, ["r2", "n2", "vcxo", "vcxo_doubler", "vco"])
        # for key in cfg["output_clocks"]:
        #     cfg["output_clocks"][key] = self.make_ints(cfg["output_clocks"][key], ["rate", "divider"])
        # return cfg

        cfg_for_template = {"clock": cfg, "map": {}}

        # Map dividers generically since we don't know how they are connected
        for chan, key in enumerate(cfg["output_clocks"]):
            cfg_for_template["map"][str(chan)] = {}
            cfg_for_template["map"][str(chan)]["source_port"] = str(chan)
            cfg_for_template["map"][str(chan)]["divider"] = cfg["output_clocks"][key][
                "divider"
            ]

        return self.gen_dt_str(clock=cfg_for_template)


class hmc7044_dt(dt, clock_dt, hmc7044_dt_layout):
